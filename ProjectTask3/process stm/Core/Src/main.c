...
/* USER CODE BEGIN PV */
#define moving_length 2
uint8_t average_array[moving_length] = {0};
...
void HAL_SPI_RxCpltCallback(SPI_HandleTypeDef *hspi){
	if (is_sendValid == 1){
		change_array(average_array, moving_length, audio_buffer);
		uint8_t val = get_average(average_array, moving_length);
		HAL_UART_Transmit(&huart2, &val, 1, HAL_MAX_DELAY);
	}
}

void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
	if (huart == &huart2){
		HAL_UART_Receive_IT(&huart2, &comp_buffer, sizeof(comp_buffer));
		if (comp_buffer == '1'){
		...
		//if(is_sendValid == 1){
			...
			//HAL_UART_Transmit(&huart2, &val, 1, HAL_MAX_DELAY);
		//}
		//HAL_UART_Receive_IT(&huart1, &audio_buffer, sizeof(audio_buffer));
...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart2, &comp_buffer, sizeof(comp_buffer));
  //HAL_UART_Receive_IT(&huart1, &audio_buffer, sizeof(audio_buffer));
  HAL_SPI_Receive_IT(&hspi1, &audio_buffer, sizeof(audio_buffer));
